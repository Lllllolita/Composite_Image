...
bg_img_path = "Images\\003997.jpg"
out_cluster_path = "Output_cluster\\003997_yh.jpg"
# out_cluster_path = "Output_cluster\\003997_xy.jpg"
# out_cluster_path = "Output_cluster\\003997_xyh.jpg"
fg_img_path = "fg\\fg.jpg"
mask_img_path = "fg\\mask.jpg"
out_result_path = "Output_result\\003997_3.jpg"
box_path = "Annotations\\003997.jpg.txt"
...
for cluster in clusters:
    ...
    h_arr.append(mean_h)
# cluster_bg_img.show()
# cluster_bg_img.save(out_cluster_path)
...
while(cycle):
    ...
    if tar_cluster == sorted_h_arr[0]:
        ...
        other_cluster_data = dict()
        for i in other_clusters_idx:
            data_idx = np.where(yhat == i)
            other_cluster_data[i] = np.take(box_arr, data_idx, axis=0)[0]
        ...
        for i, d in other_cluster_data.items():
        ...
        if len(overlap_clusters_idx) == 0:
            ...
            for i in overlap_clusters_idx:
                ...
                k_sequence = [ii + 1 for ii in range(k_num)]
                    ...
                    if flag:
                        ...
                        if len(right_x1_list) > 0 and len(left_x2_list) > 0:
                            ...
                            if str(j) not in min_x_range.keys():
                                ...
                                min_x_range[str(j)][0] = max(min_x_range[str(j)][0], np.min(tar_points[:, 1]))
                                min_x_range[str(j)][1] = min(min_x_range[str(j)][1], np.max(tar_points[:, 2]))
                no_lapped_points.append(cur_no_lapped_points)
            no_lapped_points_arr = [np.array(l) for l in no_lapped_points]
            no_lapped_points_final = reduce(np.intersect1d, no_lapped_points_arr)
            if len(no_lapped_points_final) > 0:
                print("stage3")
                cycle = False
                tar_point_idx = random.choice(no_lapped_points_final)
                tar_point = tar_points[tar_point_idx]
                res_h = int(tar_point[5] * random.uniform(1, 1.1))
                # res_y = min(int(tar_point[7] + random.uniform(0, res_h * 0.1)), np.max(tar_points[:, 7]))
                res_y = np.max(tar_points[:, 7])
